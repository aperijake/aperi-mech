...
std::vector<FieldData> GetFieldData(bool uses_generalized_fields, bool use_strain_smoothing, bool add_debug_fields) {
    ...
    field_data.push_back(FieldData("displacement_np1_temp", FieldDataRank::VECTOR, FieldDataTopologyRank::NODE, 1, std::vector<double>{}, false));    // The temporary displacement field, used in the power method calculation
    field_data.push_back(FieldData("eigenvector", FieldDataRank::VECTOR, FieldDataTopologyRank::NODE, 1, std::vector<double>{}, false));              // The eigenvector field, used as output from the power method calculation
    field_data.push_back(FieldData("force_coefficients_temp", FieldDataRank::VECTOR, FieldDataTopologyRank::NODE, 1, std::vector<double>{}, false));  // The temporary force field, used in the power method calculation
