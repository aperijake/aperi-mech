...
ssh -T -o ConnectTimeout=10 "${VM_USERNAME}@${VM_IP}" <<'EOF'
  ...
  docker-compose -f $compose_file run --rm $service_name /bin/bash -c '
    ...
    echo "Running: mv performance_aperi_mech_all_results.json ~/aperi-mech_host/build/performance_aperi_mech_all_results.json"
    mv performance_aperi_mech_all_results.json ~/aperi-mech_host/build/performance_aperi_mech_all_results.json
    ...
    for subdir in "${subdirs[@]}"; do
        ...
        for type in "${types[@]}"; do
            ...
            echo "Running: mv ${this_filename} ~/aperi-mech_host/build/${this_filename}"
            mv ${this_filename} ~/aperi-mech_host/build/${this_filename}
        done
        cd ..
    done
