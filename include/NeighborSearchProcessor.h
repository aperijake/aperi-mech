...
template<aperi::FieldDataRank FieldDataRankType>
class NeighborSearchProcessor {
   ...
   public:
    NeighborSearchProcessor(std::shared_ptr<aperi::MeshData> mesh_data, const std::vector<std::string> &sets = {}) : m_mesh_data(mesh_data), m_sets(sets) {
        ...
        m_num_neighbors_field = StkGetField(FieldQueryData{"num_neighbors", FieldQueryState::None, FieldDataRankType}, meta_data);
        m_ngp_num_neighbors_field = &stk::mesh::get_updated_ngp_field<double>(*m_num_neighbors_field);

        // Get the neighbors field
        m_neighbors_field = StkGetField(FieldQueryData{"neighbors", FieldQueryState::None, FieldDataRankType}, meta_data);
    ...
    void add_ring_0_nodes() {
        auto ngp_mesh = m_ngp_mesh;
        // Get the ngp fields
        auto ngp_num_neighbors_field = *m_ngp_num_neighbors_field;
        auto ngp_neighbors_field = *m_ngp_neighbors_field;

        // If this is an element, add the nodes attached to the element
        if constexpr ((FieldDataRankType == FieldDataRank::ELEMENT)) {
            stk::mesh::for_each_entity_run(
                ...
                KOKKOS_LAMBDA(const stk::mesh::FastMeshIndex &elem_index) {
                    ...
                    double starting_num_nodes = ngp_num_neighbors_field(elem_index, 0);
                    ngp_num_neighbors_field(elem_index, 0) += num_nodes;

                    for (size_t i = 0; i < num_nodes; ++i) {
                        size_t index = starting_num_nodes + i;
                        ngp_neighbors_field(elem_index, index) = (double)nodes[i].local_offset();
                    }
                });
        } else if constexpr ((FieldDataRankType == FieldDataRank::NODE)) {
            // If this is a node, add itself to the neighbors field
            ...
            stk::mesh::for_each_entity_run(
                ...
                KOKKOS_LAMBDA(const stk::mesh::FastMeshIndex &node_index) {
                    ...
                    double starting_num_nodes = ngp_num_neighbors_field(node_index, 0);
                    ngp_num_neighbors_field(node_index, 0) += 1;
                    ngp_neighbors_field(node_index, (size_t)starting_num_nodes) = (double)node.local_offset();
                });

        } else {
            Kokkos::abort("FieldDataRankType must be FieldDataRank::ELEMENT or FieldDataRank::NODE.");
        }
    }

    void MarkAndSyncFieldsToHost() {
        m_ngp_neighbors_field->modify_on_device();
        m_ngp_num_neighbors_field->modify_on_device();
        m_ngp_neighbors_field->sync_to_host();
        m_ngp_num_neighbors_field->sync_to_host();
    }

    void MarkAndSyncFieldsToDevice() {
        m_ngp_neighbors_field->modify_on_host();
        m_ngp_num_neighbors_field->modify_on_host();
        m_ngp_neighbors_field->sync_to_device();
        m_ngp_num_neighbors_field->sync_to_device();
    }

    void CommunicateAllFieldData() const {
        stk::mesh::communicate_field_data(*m_bulk_data, {m_neighbors_field, m_num_neighbors_field});
    }
