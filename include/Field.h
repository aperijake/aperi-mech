...
class Field {
   public:
    Field(const aperi::MeshData &mesh_data, const aperi::FieldQueryData<T> &field_query_data)
        : m_field(stk::mesh::get_updated_ngp_field<T>(*StkGetField(field_query_data, mesh_data.GetMetaData()))) {
    }
    ...
    KOKKOS_FUNCTION Eigen::Matrix<T, N, M> GetEigenMatrix(const aperi::Index &index) const {
        ...
        for (size_t i = 0; i < N; ++i) {
            for (size_t j = 0; j < M; ++j) {
                data(i, j) = m_field(fast_index, i * M + j);
            }
        }
    }
    ...
    KOKKOS_FUNCTION Eigen::Map<Eigen::Matrix<T, N, M>, 0, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> GetEigenMap(const aperi::Index &index) const {
        const unsigned component_stride = m_field.get_component_stride();
        Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic> stride(component_stride, M * component_stride);
        return Eigen::Map<Eigen::Matrix<T, N, M>, 0, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>>(&m_field(index(), 0), stride);
    }
    ...
    KOKKOS_FUNCTION void AtomicAdd(const aperi::Index &index, const Eigen::Matrix<T, N, M> &data) const {
        ...
        for (size_t i = 0; i < N; ++i) {
            for (size_t j = 0; j < M; ++j) {
                Kokkos::atomic_add(&m_field(fast_index, i * M + j), data(i, j));
            }
        }
    }
    ...
    KOKKOS_FUNCTION void Add(const aperi::Index &index, const Eigen::Matrix<T, N, M> &data) const {
        ...
        for (size_t i = 0; i < N; ++i) {
            for (size_t j = 0; j < M; ++j) {
                m_field(fast_index, i * M + j) += data(i, j);
            }
        }
    }
    ...
    KOKKOS_FUNCTION void AtomicOverwrite(const aperi::Index &index, const Eigen::Matrix<T, N, M> &data) const {
        ...
        for (size_t i = 0; i < N; ++i) {
            for (size_t j = 0; j < M; ++j) {
                Kokkos::atomic_assign(&m_field(fast_index, i * M + j), data(i, j));
            }
        }
    }
   ...
   private:
    stk::mesh::NgpField<T> m_field;  // The ngp field object.
};
