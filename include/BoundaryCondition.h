...
class BoundaryCondition {
   public:
    BoundaryCondition(std::vector<std::pair<size_t, double>> components_and_values, std::pair<std::function<double(double)>, std::function<double(double)>> time_functions, std::vector<std::string> sets, std::shared_ptr<aperi::MeshData> mesh_data) : m_components_and_values(components_and_values), m_velocity_time_function(time_functions.first), m_acceleration_time_function(time_functions.second) {
        const std::array<FieldQueryData, 1> velocity_field_query_data_vec = {FieldQueryData{"velocity", FieldQueryState::NP1}};
        const std::array<FieldQueryData, 1> acceleration_field_query_data_vec = {FieldQueryData{"acceleration", FieldQueryState::NP1}};
        m_node_processor_velocity = std::make_shared<aperi::NodeProcessorStkNgp<1>>(velocity_field_query_data_vec, mesh_data, sets);
        m_node_processor_acceleration = std::make_shared<aperi::NodeProcessorStkNgp<1>>(acceleration_field_query_data_vec, mesh_data, sets);
    }
   ...
   private:
    ...
    std::shared_ptr<aperi::NodeProcessorStkNgp<1>> m_node_processor_velocity;
    std::shared_ptr<aperi::NodeProcessorStkNgp<1>> m_node_processor_acceleration;
};
