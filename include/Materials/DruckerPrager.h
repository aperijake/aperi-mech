 ...
 */
class DruckerPragerPlasticity : public Material {
   public:
    /**
     * @brief Constructor for DruckerPragerPlasticity class.
     * @param material_properties The properties
     */
    DruckerPragerPlasticity(std::shared_ptr<MaterialProperties> material_properties) : Material(material_properties) {
        CreateStressFunctor();
    }
    ...
    void CreateStressFunctor() {
        ...
        Kokkos::parallel_for(
            "CreateObjects", 1, KOKKOS_LAMBDA(const int&) {
                new ((DruckerPragerGetStressFunctor*)stress_functor) DruckerPragerGetStressFunctor(lambda, two_mu, yield_stress, hardening_modulus);
            });
    ...
    struct DruckerPragerGetStressFunctor : public StressFunctor {
        ...
        KOKKOS_FUNCTION
        DruckerPragerGetStressFunctor(double lambda, double two_mu, double yield_stress, double hardening_modulus) : m_lambda(lambda), m_mu(two_mu / 2.0), m_yield_stress(yield_stress), m_hardening_modulus(hardening_modulus) {}
        ...
        void GetStress(const Eigen::Map<const Eigen::Matrix<double, 3, 3>, 0, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>>* displacement_gradient_np1,
            ...
            double volumetric_plastic_strain = state_old->coeff(VOLUMETRIC_PLASTIC_STRAIN);
            ...
            state_new->coeffRef(VOLUMETRIC_PLASTIC_STRAIN) = volumetric_plastic_strain;
