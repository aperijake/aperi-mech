...
run_and_cleanup() {
	...
	echo "Running: ./batch_run.sh -n 1 -e ~/aperi-mech/protego-mech/build/ -p 4,2,1 $1"
	./batch_run.sh -n 1 -e ~/aperi-mech/protego-mech/build/ -p 4,2,1 "$1"
	cd "$1" || exit
	mkdir azuregpu2_results
	mv run_* azuregpu2_results
	rm results.exo
	cd "${current_dir}" || exit
}
...
# run_and_cleanup taylor_bar_1e6_hexes/rkpm_nodal # Fails due to memory
