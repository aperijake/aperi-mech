...
template <int N>
void TestSvdInverse(const Eigen::Matrix<double, N, N> &A) {
    // Create a Kokkos::View and copy A to device
    Kokkos::View<double *> A_device("A_device", N * N);
    Kokkos::View<double *>::HostMirror A_host = Kokkos::create_mirror_view(A_device);
    for (int j = 0; j < N * N; ++j) {
        A_host(j) = A(j / N, j % N);
    }
    Kokkos::deep_copy(A_device, A_host);

    Kokkos::View<double *> Ainv_device("Ainv_device", N * N);
    Kokkos::View<double *>::HostMirror Ainv_host = Kokkos::create_mirror_view(Ainv_device);

    Kokkos::parallel_for(
        "SvdInverse", 1, KOKKOS_LAMBDA(const int &) {
            Eigen::Matrix<double, N, N> A_device_matrix;
            for (int j = 0; j < N * N; ++j) {
                A_device_matrix(j / N, j % N) = A_device(j);
            }

            Eigen::Matrix<double, N, N> Ainv_matrix;
            // Compute SVD pseudo-inverse on device
            Ainv_matrix = aperi::SvdPseudoInverse(A_device_matrix);

            for (int j = 0; j < N * N; ++j) {
                Ainv_device(j) = Ainv_matrix(j / N, j % N);
            }
        });

    Kokkos::deep_copy(Ainv_host, Ainv_device);
    Eigen::Matrix<double, N, N> Ainv;
    for (int j = 0; j < N * N; ++j) {
        Ainv(j / N, j % N) = Ainv_host(j);
    }

    Eigen::Matrix<double, N, N> Ainv_eigen = A.completeOrthogonalDecomposition().pseudoInverse();

    EXPECT_TRUE(Ainv.isApprox(Ainv_eigen, 1e-10)) << "Matrix: \n"
                                                  << A << "\nSVD Inverse:\n"
                                                  << Ainv << "\nEigen Inverse:\n"
                                                  << Ainv_eigen << "\nDifference:\n"
                                                  << Ainv - Ainv_eigen;
...
TEST(MathUtilsTest, SVDInverse) {
    // Compare SVD Inverse vs Eigen on a bunch of random matrices
    // Seed random number generator for reproducibility
    std::srand(42);

    size_t n = 10;  // number of random matrices for each size

    // Test 2x2
    for (int i = 0; i < n; ++i) {
        const Eigen::Matrix2d A2 = Eigen::Matrix2d::Random();
        TestSvdInverse(A2);
    }

    // Test 3x3
    for (int i = 0; i < n; ++i) {
        const Eigen::Matrix3d A3 = Eigen::Matrix3d::Random();
        TestSvdInverse(A3);
    }

    // Test 4x4
    for (int i = 0; i < n; ++i) {
        const Eigen::Matrix4d A4 = Eigen::Matrix4d::Random();
        TestSvdInverse(A4);
    }

    // Test 5x5
    for (int i = 0; i < n; ++i) {
        const Eigen::Matrix<double, 5, 5> A5 = Eigen::Matrix<double, 5, 5>::Random();
        TestSvdInverse(A5);
    }

    // Test 10x10
    for (int i = 0; i < n; ++i) {
        const Eigen::Matrix<double, 10, 10> A10 = Eigen::Matrix<double, 10, 10>::Random();
        TestSvdInverse(A10);
    }
}