...
def create_gmsh_cylinder(height, radius, mesh_size, out_file_base):
    ...
    bottom_circle = gmsh.model.getEntitiesInBoundingBox(
        -radius, -radius, 0, radius, radius, 0, 0
    )
    top_circle = gmsh.model.getEntitiesInBoundingBox(
        -radius, -radius, height, radius, radius, height, 0
    )
...
def convert_gmsh_to_exo(out_file_base, cleanup=True):
    # Convert the Gmsh mesh to ExodusII format using meshio
    mesh = meshio.read(out_file_base + ".msh")
    meshio.write(out_file_base + "_meshio.e", mesh)
    if cleanup:
        os.remove(out_file_base + ".msh")


def add_nodeset_and_fix_exo(out_file_base, cleanup=True):
    ...
    exo_out = exodus.exodus(
        out_file,
        mode="w",
        array_type="numpy",
        title="Cylinder Mesh",
        numDims=3,
        numNodes=points.shape[0],
