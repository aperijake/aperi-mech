...
if len(sys.argv) != 3:
    print("Usage: python rename_exodus_fields.py <file> <output_file>")
    sys.exit(1)
...
old_file = exodus.exodus(file_name, mode="r", array_type="numpy")
...
for var in old_nodal_vars:
    ...
    if var.startswith("force_local_"):
        new_var = var.replace("force_local_", "force_")
        nodal_vars["old"].append(var)
        nodal_vars["new"].append(new_var)
        print(f"Renaming nodal variable: {var} to {new_var}")
    else:
        nodal_vars["old"].append(var)
        nodal_vars["new"].append(var)

new_file.set_node_variable_number(len(nodal_vars["new"]))
for index, vars in enumerate(zip(nodal_vars["old"], nodal_vars["new"]), start=1):
    print(f"Processing nodal variable: {vars[0]} to {vars[1]}")
    new_file.put_node_variable_name(vars[1], index)
    for time_step in range(1, old_file.num_times() + 1):
        values = old_file.get_node_variable_values(vars[0], time_step)
        new_file.put_node_variable_values(vars[1], time_step, values)
